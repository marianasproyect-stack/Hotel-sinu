...
SELECT nextval('rol_seq'), 'CLIENTE'
WHERE NOT EXISTS (SELECT 1 FROM ROLES WHERE NOMBRE = 'CLIENTE');

INSERT INTO ROLES (ID, NOMBRE)
SELECT nextval('rol_seq'), 'ADMINISTRADOR'
...
SELECT nextval('habitacion_seq'), '101', 'Sencilla', 2, 180000, 'Habitacion sencilla con cama doble, ideal para una o dos personas.', 'DISPONIBLE'
WHERE NOT EXISTS (SELECT 1 FROM HABITACIONES WHERE NUMERO = '101');

INSERT INTO HABITACIONES (ID, NUMERO, TIPO, CAPACIDAD, PRECIO_NOCHE, DESCRIPCION, ESTADO)
SELECT nextval('habitacion_seq'), '102', 'Doble', 3, 240000, 'Habitacion doble con dos camas y espacio extra para una tercera persona.', 'DISPONIBLE'
WHERE NOT EXISTS (SELECT 1 FROM HABITACIONES WHERE NUMERO = '102');

INSERT INTO HABITACIONES (ID, NUMERO, TIPO, CAPACIDAD, PRECIO_NOCHE, DESCRIPCION, ESTADO)
SELECT nextval('habitacion_seq'), '201', 'Suite', 4, 380000, 'Suite con sala separada, balcon y minibar.', 'DISPONIBLE'
WHERE NOT EXISTS (SELECT 1 FROM HABITACIONES WHERE NUMERO = '201');

INSERT INTO HABITACIONES (ID, NUMERO, TIPO, CAPACIDAD, PRECIO_NOCHE, DESCRIPCION, ESTADO)
SELECT nextval('habitacion_seq'), '202', 'Suite Presidencial', 4, 620000, 'La habitacion mas grande del hotel, con terraza privada.', 'DISPONIBLE'
